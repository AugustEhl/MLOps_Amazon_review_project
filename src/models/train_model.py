    ...
    with wandb.init(config=config):
      ...
      for i in range(config.epochs):
        ...
        for batch_idx, data in enumerate(trainloader):
          ...
          running_acc += (((y_pred == labels).sum() / labels.shape[0]) / len(trainloader)).item()
          wandb.log({'Epoch_' + str(i+1) + ' (Batch loss)': running_loss, 'Epoch_' + str(i+1) + ' (Batch accuracy)': running_acc})
          if ((batch_idx+1) % 5) == 0:
        ...
        print(f'Epoch {i+1} loss: {running_loss / len(trainloader)} \tEpoch acc.: {running_acc}')
        wandb.log({'Loss': running_loss / len(trainloader),
          'Accuracy (%)': running_acc,
          'Classes': table})
      torch.save(model, f'models/model_opt{config.optimizer}_bs{config.batch_size}_do{config.drop_out}_lr{config.lr}.pth')
